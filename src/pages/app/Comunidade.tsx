...
const Comunidade = () => {
  ...
  return (
    <AppShell accent={accent}>
      ...
      <section className="mb-6 flex items-center gap-3 sm:gap-4 min-w-0">
        <div className={`size-12 sm:size-14 rounded-2xl ${accentBgSoft} ${accentText} flex items-center justify-center text-2xl sm:text-3xl shrink-0`}>
          {active.emoji}
        </div>
        <div className="min-w-0">
          <h1 className="font-display font-bold text-xl sm:text-3xl tracking-[-0.03em] truncate">{active.name}</h1>
          <p className="text-xs sm:text-sm text-ink/55">Chat aberto a alunos e mentores</p>
        </div>
      </section>

      <div className="bg-paper border border-ink/8 rounded-3xl overflow-hidden flex flex-col h-[70vh] min-h-[420px] sm:min-h-[480px]">
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-4">
          {messages.length === 0 ? (
            ...
            messages.map((m) => {
              ...
              return (
                <div key={m.id} className={`flex gap-2 sm:gap-3 ${mine ? "flex-row-reverse" : ""}`}>
                  <div className={`size-8 sm:size-9 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-semibold shrink-0 ${roleBadgeCls}`}>
                    {initialsOf(m.author?.full_name)}
                  </div>
                  <div className={`max-w-[80%] sm:max-w-[75%] min-w-0 ${mine ? "items-end" : "items-start"} flex flex-col`}>
                    <div className={`flex items-center gap-1.5 sm:gap-2 mb-1 text-[10px] sm:text-[11px] flex-wrap ${mine ? "flex-row-reverse" : ""}`}>
                      <span className="font-semibold text-ink truncate max-w-[110px] sm:max-w-[140px]">
                        {mine ? "Você" : m.author?.full_name ?? "Usuário"}
                      </span>
                      <span className={`inline-flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] uppercase tracking-[0.14em] sm:tracking-[0.16em] font-semibold ${roleBadgeCls}`}>
                        <RoleIcon className="size-2.5" />
                        {isMentor ? "Mentor" : "Aluno"}
                      </span>
                      <span className="text-ink/40 text-[10px]">{fmtTime(m.created_at)}</span>
                    </div>
